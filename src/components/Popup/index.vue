	...
	<div class="popup" :class="{'playMode': playModeVisible}">
		...
		<mt-popup class="playListSheet" v-model="playListVisible" position="bottom">
			...
			<ul class="playList" v-if="playList && playList.length > 0">
				<li class="playList-item" v-for="(item, index) in playList" :key="item.sound.id" :class="{'playing': audio.data.sound.id === item.sound.id}" @click="musicChange(item)">
					<div class="item-name">
						<!-- icon	 -->
						...
						<!-- 播放列表歌名 -->
						<div class="name-value" :class="audio.data.sound.id ? 'onPlay': ''">{{item.sound.name}}</div>
					</div>
					<!-- 删除歌曲按钮 -->
					<div class="item-close my-icon-close" @click.stop="deletePlayListItem(item, index)"></div>  <!-- @click.stop 阻止事件冒泡 -->
				</li>
			</ul>
			<div class="playList-nothing" v-else>这里空空的哦~</div>
		</mt-popup>
